...
if(isNil "JSt4r_open_notifications") then {
    JSt4r_open_notifications = [];
};
...
private _TextField = _display ctrlCreate ["RscStructuredText", -1];
	...
	if (count JSt4r_open_notifications > 0) then {
	...
	} forEach JSt4r_open_notifications;
	};
JSt4r_open_notifications = ([[_Header,_TextField]] + JSt4r_open_notifications) select {!isNull (_x select 0) && !isNull (_x select 1)}; // Add the Element to the Front of the Array and remove the deleted Arrays